...
parser.add_argument("-a","--align", type=int, default = 1, help = "Align the PMF to the methane potential for that surface")
parser.add_argument("-o","--offset", type=float, default = 0.1, help = "Standard deviation for the random offset")
...
allowAlign =  args.align

if args.offset > 0.01:
    offsetMag = args.offset
else:
    offsetMag = 0
...
pmfOutputFile = open("Datasets/PMFCoefficients-N"+str(numReplicas)+noiseStr+"-oct12.csv","w")

#PMFMethaneOffset records the total offset under the assumption that PMFMethaneOffset = 0 implies alignment
...
for target in targetSet[args.initial::args.step]:
    ...
    for i in range(numReplicas):
        ...
        for r0Val in r0ValRange:
            if numReplicas > 1:
                if offsetMag > 0.0001:
                    randomOffset =   np.random.normal( 0, offsetMag)    #(np.random.random() - 0.5)*0.2 #map x~U[0,1] to x~[-0.5,0.5] to x~[-0.1,0.1]
                else:
                    randomOffset = 0
            ...
            outputOffset = totalOffsetApplied + materialOffsetVal #redefine the output offset relative to the canonical methane offset
